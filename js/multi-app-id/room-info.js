define([
  'jquery', 'lodash', 'multi-app-id/content', 'multi-app-id/route',
  'multi-app-id/qiscus'
  ...
  var loadContact = _.debounce(function (currentLength) {
    ...
    isLoadingUser = true
    qiscus.getUsers(query, nextPage, perPage, function (users) {
      isLoadingUser = false
  ...
  function ContactChooser() {
    qiscus.getUsers('', 1, 10, function (users) {
      users = users.map(function (user) {
  ...
  function RoomInfo(state) {
    qiscus.getChatRooms([state.roomId], 1, false, true, function (rooms, err) {
  ...
  function addParticipant(detail) {
    ...
    var $el = $content.find(`li.contact-item[data-contact-userid="${detail.id}"]`)
    $el
  ...
  $content
    ...
    .on('keydown', '.RoomInfo #input-room-name', function (event) {
      if (event.keyCode === 13) {
        ...
        qiscus.updateChatRoom(roomId, name, void 0, void 0, function (room) {
          // do nothing
        })
      }
    })
    .on('change', '.RoomInfo #input-avatar', function (event) {
      ...
      qiscus.upload(file, function (err, progress, url) {
        ...
        if (url) {
          var roomId = route.location.state.roomId
          qiscus.updateChatRoom(roomId, void 0, url, void 0, function (room) {
            // do nothing
          })
        }
      })
    ...
    .on('click', '.RoomInfo #remove-participant-btn', function (event) {
      ...
      qiscus.removeParticipants(roomId(), [userId], function (_, error) {
    ...
    .on('click', '.RoomInfo #add-participant-btn', function (event) {
      event.preventDefault()
      qiscus.addParticipants(roomId(), selectedIds, function (users, err) {
    ...
    .on('input', '.RoomInfo #search', function (event) {
      ...
      return qiscus.getUsers(searchQuery, 1, 10, function (users, err) {
